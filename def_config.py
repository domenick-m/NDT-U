...
config.model.mask_ratio = 0.65 # Percentage of tokens being used to compute the loss are zero masked
...
config.wandb.project = 'Single Session Sweep' # The wandb project the run should be stored in
...
config.wandb.sweep.model.mask_ratio = [0.5, 0.75, 0.95]
...
config.wandb.sweep.model.n_layers = [4, 6]
config.wandb.sweep.model.hidden_size = [64, 128, 256]
# # config.wandb.sweep.model.factor_dim = [16, 32, 64, 128, 256]
config.wandb.sweep.model.dropout = [0.3, 0.5, 0.7]
config.wandb.sweep.model.dropout_rates = [0.3, 0.5, 0.7]
config.wandb.sweep.model.dropout_embedding = [0.3, 0.5, 0.7]
