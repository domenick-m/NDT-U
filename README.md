...
# Usage
To train a model simply run: `python train.py`, the system will then download and extract the selected dataset. The system will automatically use the configurations from my submissons (currently only `mc_maze_small`, `mc_maze_medium`, `mc_maze_large`, and `area2_bump`) unless the `--default` flag is used, then it will only use the configs in `default_config.py`. To run a sweep you can run: `python train.py --sweep`. To add an agent to an already running sweep run: `python train.py --add`. 

You may also set configs for the current run by running: `python train.py --CONFIG_TO_SET value_to_change_to`. Example: `python train.py --dataset mc_maze --epochs 1000`
