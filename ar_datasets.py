...
def chop(data, width):
    shape = (int((data.shape[0] - (width - 1))), width, data.shape[-1])
    strides = (data.strides[0], data.strides[0], data.strides[1])
    return np.lib.stride_tricks.as_strided(data, shape=shape, strides=strides).copy().astype('f')
        ...
        with h5py.File(filename, 'r') as h5file:
            ...
            if mode == 'train':
                ...
                chopped_heldin = chopped_heldin.reshape((
                    ...
                    chopped_heldin.shape[2], 
                    chopped_heldin.shape[3]
                ))
                self.spikes_heldin = torch.tensor(chopped_heldin)
                ...
                chopped_heldout = chopped_heldout.reshape((
                    ...
                    chopped_heldout.shape[2], 
                    chopped_heldout.shape[3]
                ))
                self.spikes_heldout = torch.tensor(chopped_heldout)
